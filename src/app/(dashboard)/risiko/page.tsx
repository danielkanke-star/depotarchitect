...
export default async function RisikoPage() {
  const { portfolio, settings, positions, categories } = await getPortfolioData();
  const calculation = calculatePortfolio({ netLiquidity: portfolio.net_liquidity, positions: positions.map((position) => positionToCalculationInput(position, portfolio, categories)) });
