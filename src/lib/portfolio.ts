...
import type { Portfolio, PortfolioCashBalance, PortfolioCategory, PortfolioImport, PortfolioSettings, Position } from "@/lib/database.types";
...
export async function getPortfolioData(): Promise<{
  ...
  const [{ data: settings, error: settingsError }, { data: categories, error: categoriesError }, { data: positions, error: positionsError }, { data: cashBalances, error: cashError }, { data: latestImport, error: importError }] = await Promise.all([
  ...
  return { portfolio, settings, categories, positions, cashBalances, latestImport };
}
import "server-only";
