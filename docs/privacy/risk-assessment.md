...
Der erneute npm-Audit für 2B.5 führt zusätzlich die transitive `brace-expansion`-DoS-Warnung ohne verfügbaren automatischen Fix auf. Der betroffene Pfad gehört zur lokalen ESLint-/Build-Werkzeugkette; die Anwendung verarbeitet darüber keine vom Benutzer gelieferten Glob-Muster zur Laufzeit. Die Abhängigkeit bleibt bis zu einem kompatiblen Upstream-Update zu beobachten. Die 20 als „high“ gezählten Pakete beruhen auf diesen beiden transitiven Wurzelhinweisen (`sharp` und `brace-expansion`), nicht auf 20 unabhängigen Laufzeitlücken.
